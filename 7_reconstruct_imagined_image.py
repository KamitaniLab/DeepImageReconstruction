...
# use leaky ReLU layers and average pooling layers
prototxt_file = './net/VGG_ILSVRC_19_layers/VGG_ILSVRC_19_layers_lrelu_avePool.prototxt'
channel_swap = (2, 1, 0)
...
# the image used in Figure 4, the image label can be from 1 to 26 for imagined images in test data
image_label_list = range(1, 27)
        ...
        for image_label in image_label_list:  # loop for image label
            ...
            for layer in layers:
                file_name = os.path.join(feat_dir, stim_type, net_name, layer, subject, ROI, stim_type+'-'+net_name+'-'+layer+'-'+subject +
                                         '-'+ROI+'-Img%04d.mat' % image_label)  # the full file name depends on the data structure for decoded CNN features
