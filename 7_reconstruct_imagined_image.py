# Here the codes are used to reconstruct imagined image from the CNN features decoded from brain;
# conditions:
# ROI: VC
# layers: use all conv and fc layers
# without DGN
# CNN: VGG19 with leaky ReLU layers and average pooling layers
...
prototxt_file = './net/VGG_ILSVRC_19_layers/VGG_ILSVRC_19_layers_lrelu_avePool.prototxt' # use leaky ReLU layers and average pooling layers
channel_swap = (2,1,0)
...
# feature std estimated from true CNN (VGG19 with leaky ReLU layers and average pooling layers) features of 10000 images
feat_std_file = './data/estimated_vgg19LeakyReluAvePool_cnn_feat_std.mat'
...
stim_type = 'imagery' # for imagined image

net_name = 'VGG19_leaky'
...
image_label_list = range(1,27) # the image used in Figure 4, the image label can be from 1 to 26 for imagined images in test data
...
for subject in subjects: # loop for subjects
    for ROI in ROIs: # loop for ROIs
        for image_label in image_label_list: # loop for image label
            ...
            features = {}
            for layer in layers:
                file_name = os.path.join(feat_dir,stim_type,net_name,layer,subject,ROI,stim_type+'-'+net_name+'-'+layer+'-'+subject+'-'+ROI+'-Img%04d.mat'%image_label) # the full file name depends on the data structure for decoded CNN features
